...
/**
 * Used by both inline-params and store-payload demos.
 * The category-specific extra field is shown conditionally via @ModalWhen.
 */
@ModalDynamic<TicketModalData>()
@ModalComponent({ id: 'ticket_inline', title: 'Open a Ticket' })
export class TicketInlineModal {
  @ModalInput({ label: 'Subject', placeholder: 'Brief summary of your issue', required: true, maxLength: 100 })
  subject!: string;

  @ModalInput({ label: 'Description', placeholder: 'Describe the issue in detail', style: 'paragraph', required: true })
  description!: string;
