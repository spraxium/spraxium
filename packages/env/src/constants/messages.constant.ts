...
export const MESSAGES = {
  ...
  DESC_LOADED_VARS:
    " Loaded environment variables for the current runtime context.",
  ...
  VALIDATION_ERROR: (count: number) =>
    `Environment validation failed with ${count} error(s)`,
  ...
  EXPECTED_EMAIL: (value: unknown) =>
    `Expected a valid email address, got "${value}"`,
  EXPECTED_HOST: "Expected a hostname or IP address",
  EXPECTED_HOST_INVALID: (value: unknown) =>
    `Expected a valid hostname or IP address, got "${value}"`,
  EXPECTED_MONGO_URI:
    'Expected a MongoDB URI starting with "mongodb://" or "mongodb+srv://"',
  EXPECTED_REDIS_URI:
    'Expected a Redis URI starting with "redis://" or "rediss://"',
  ...
  DISCORD_TOKEN_LENGTH: (length: number) =>
    `Discord token length out of range (got ${length})`,
