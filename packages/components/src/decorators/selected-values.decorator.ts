...
export function SelectedValues(): ParameterDecorator {
  return (target: object, propertyKey: string | symbol | undefined, parameterIndex: number): void => {
    ...
    const existing: number[] =
      Reflect.getMetadata(METADATA_KEYS.SELECT_SELECTED_VALUES_PARAM, target, key) ?? [];
    existing.push(parameterIndex);
    Reflect.defineMetadata(METADATA_KEYS.SELECT_SELECTED_VALUES_PARAM, existing, target, key);
  };
}
