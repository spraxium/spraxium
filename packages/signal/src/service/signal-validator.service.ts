...
export class SignalValidator {
  validate(message: Message, config: SignalConfig, knownEvents: ReadonlySet<string>): ValidationResult {
    ...
    if (dbg)
      log.info(
        SIGNAL_MESSAGES.V_INCOMING(
          message.id,
          message.channelId,
          message.webhookId ?? 'none',
    ...
    if (dbg) log.info(SIGNAL_MESSAGES.V_ALL_OK(envelope.event));
    return { ok: true, envelope };
  }

  private validateSource(message: Message, config: SignalConfig, dbg: boolean): boolean {
    if (!message.webhookId) {
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R1_FAIL);
      return false;
    }
    if (dbg) log.info(SIGNAL_MESSAGES.V_R1_OK(message.webhookId));

    if (!config.allowedWebhookIds.includes(message.webhookId)) {
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R2_FAIL(message.webhookId, config.allowedWebhookIds.join(', ')));
      return false;
    }
    if (dbg) log.info(SIGNAL_MESSAGES.V_R2_OK);

    const channels = Array.isArray(config.channelId) ? config.channelId : [config.channelId];
    if (!channels.includes(message.channelId)) {
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R3_FAIL(message.channelId, channels.join(', ')));
      return false;
    }
    if (dbg) log.info(SIGNAL_MESSAGES.V_R3_OK);
  ...
  private parseJson(content: string, dbg: boolean): Record<string, unknown> | null {
    ...
    try {
      ...
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R4_FAIL_JSON(content.slice(0, 200)));
      return null;
    }

    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R4_FAIL_OBJ);
      return null;
    }

    if (dbg) log.info(SIGNAL_MESSAGES.V_R4_OK);
  ...
  private validateEnvelopeFields(
    ...
    if (raw.v !== 1) {
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R5_FAIL(raw.v));
      return false;
    }
    if (dbg) log.info(SIGNAL_MESSAGES.V_R5_OK);

    if (typeof raw.event !== 'string' || !knownEvents.has(raw.event)) {
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R6_FAIL(raw.event, [...knownEvents].join(', ')));
      return false;
    }
    if (dbg) log.info(SIGNAL_MESSAGES.V_R6_OK(raw.event as string));

    if (typeof raw.guildId !== 'string' || raw.guildId.length === 0) {
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R7_FAIL(raw.guildId));
      return false;
    }
    if (dbg) log.info(SIGNAL_MESSAGES.V_R7_OK(raw.guildId as string));

    if (typeof raw.nonce !== 'string' || raw.nonce.length === 0) {
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R8_FAIL(raw.nonce));
      return false;
    }
    if (dbg) log.info(SIGNAL_MESSAGES.V_R8_OK(raw.nonce as string));
  ...
  private validateTimestamp(sentAt: unknown, dbg: boolean): boolean {
    if (typeof sentAt !== 'number') {
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R9_FAIL_TYPE(sentAt));
      return false;
    }

    const skew = Math.abs(Date.now() - sentAt);
    if (skew > MAX_CLOCK_SKEW_MS) {
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R9_FAIL_SKEW(skew, MAX_CLOCK_SKEW_MS));
      return false;
    }

    if (dbg) log.info(SIGNAL_MESSAGES.V_R9_OK(skew));
    return true;
  }
  ...
  private validateSignature(raw: Record<string, unknown>, secret: string, dbg: boolean): boolean {
    if (typeof raw.signature !== 'string' || raw.signature.length === 0) {
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R10_FAIL_MISSING);
      return false;
    }
    ...
    if (!valid) {
      if (dbg) log.warn(SIGNAL_MESSAGES.V_R10_FAIL_HMAC);
      return false;
    }

    if (dbg) log.info(SIGNAL_MESSAGES.V_R10_OK);
    return true;
  }
}
